# CHARACTER STATE REGISTRY

## E*NKI

### CHR-ENKI-A0
Ordinary rabbit before MITER-defined humanoid identity. Used for archive/memory material only.

### CHR-ENKI-E0
Transformed organic humanoid lagomorph in early Saint Juniper clinical state.
- white fur
- mature angular face
- restrained muzzle
- RIGHT eye red/orange
- LEFT eye amber/yellow
- MITER bracelet
- clinical/patient clothing where scene requires

### CHR-ENKI-E1 — HUNTER LOADOUT
Begins after forest hunter inventory sequence.
- same body/face/eyes as E0
- stolen practical jacket
- stolen pants
- stolen boots
- bag/sack and useful material
- MITER bracelet remains
- later PROP-VIAL-V1 joins carried inventory at SHOT_141
- forbidden: mustard parka/red hoodie/olive cargo/violet sneaker iconic outfit

## N*ABU

State progression is strictly:
A0 ordinary raccoon -> A1 contaminated animal -> A2 coma transformation -> A3 humanoid -> A3-T0 temporary covering.

Transformation must NOT occur at the pipe.

### CHR-NABU-A3
- organic raccoon humanoid
- compact, robust, agile
- strong natural raccoon facial mask
- articulated humanoid hands with claws
- bipedal feet
- tail retained
- no cybernetics

### CHR-NABU-A3-T0
A3 with improvised blanket/tissue/cord/strap covering from station resources. No polished costume and no techwear.

## U*TTU

State progression:
UT0 ordinary spider -> contamination/exposure -> UT1 transformed survivor.

### CHR-UTTU-UT1
- very tall/slender
- dark bronze/mineral organic skin/body
- 2 humanoid arms
- 6 arachnid limbs
- calm sovereign bearing
- biological golden silk
- no fantasy magic
- no unexplained extra limbs

## JD — CHR-JD-J1
- human
- dark bordeaux/red full balaclava/hood
- two small feline-like ears
- single horizontal eye slit
- black matte bomber + hoodie/straps
- observes before intervening
- not trio commander

## G*BOY — CHR-GBOY-G1
Internal production identity may be fully defined, but pilot presentation remains fragmentary.
- red hood with horns
- yellow T-shirt
- pale green shorts
- violet/white sneakers
- black skin
- white eyes
- not fox
- former member of The Bloc
- current nature unexplained

## Scale lock requirement

Before animation production, create and approve one neutral lineup containing:
E*NKI E1 / N*ABU A3 / U*TTU UT1 / JD J1.

That lineup becomes the relative-height reference for every multi-character shot.
